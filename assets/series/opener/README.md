...
| 3 Hopper | `03-hopper.mp4` | v4 `VLu2Z2eZCTeAN92dIq3c` | 7 s | HOPPER, ENV1 | v2 and v3 both rejected for the same continuity error: the coin stayed on his hat while a second coin appeared for the flick. The model cannot hold "take the coin off, then flick it". v4 changes the gag: the coin lands on his hat and stays there, he shrugs, and leaps with it still balanced on his head. One coin, no handling. Review PENDING. Rejected versions in `rejected/`. |
