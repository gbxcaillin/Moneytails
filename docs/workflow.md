...
- **Stage a character on the side they will end up, so they never cross a solid prop.** When a still-to-still clip moves a character from one side of a solid object to the other, the model takes the straight path and phases the character through the object (EP01 clip 16b, Hopper rising up through the fallen scooter). Fix it upstream: make the previous clip leave the character on the side they need to be, then the transition is a rise or turn in place with the prop beside them, never through them.
- **Turn audio off for a silent clip if a generated music bed trips output moderation.** Seedance can synthesize a bed (a ukulele riff, say) that its own output filter then rejects as possible audio copyright, failing the whole render (EP01 clip 16b, first pass, error output_moderation_blocked). For a clip that carries no dialogue, set generateAudio false and add the footsteps and ambience in the edit; re-mux a silent stereo track onto the file so it stays well-formed for the timeline.
